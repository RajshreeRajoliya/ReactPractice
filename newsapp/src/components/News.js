...
export class News extends Component {
 ...
 async componentDidMount() {
  let url=`https://newsapi.org/v2/top-headlines?country=in&category=business&apiKey=1fece3659b11477eb1bb901d5ae5fa69&page=1&pageSize=${this.props.pageSize}`
...
this.setState({articles: parsedData.articles,
  ...
  handlePrevClick=async() =>{
    let url=`https://newsapi.org/v2/top-headlines?country=in&category=business&apiKey=1fece3659b11477eb1bb901d5ae5fa69&page=${this.state.page - 1}&pageSize=${this.props.pageSize}`
    this.setState({loading:true})
    const data =await fetch(url)
    const parsedData = await data.json()
    this.setState({loading:false})
